...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
byte last_channel_1, last_channel_2, last_channel_3, last_channel_4, last_channel_5, last_channel_6, last_channel_7;
byte eeprom_data[36];
...
unsigned long timer_1, timer_2, timer_3, timer_4, timer_5, timer_6, timer_7, current_timeV1, current_timeV2;
...
{
	#define dbfaktor 1
	if (debcnt == 0*dbfaktor) Serial.print(start );
	if (debcnt == 2*dbfaktor) Serial.print(":" );
	if (debcnt == 4*dbfaktor) Serial.print(throttle);
	if (debcnt == 6*dbfaktor) Serial.print(" RG: ");
	if (debcnt == 8*dbfaktor) Serial.print(angle_roll);
	if (debcnt == 10*dbfaktor) Serial.print(" RA: ");
	if (debcnt == 12*dbfaktor) Serial.print(angle_roll_acc);
	if (debcnt == 14*dbfaktor) Serial.print(" SP: ");
	if (debcnt == 16*dbfaktor) Serial.print(pid_roll_setpoint);
	if (debcnt == 18*dbfaktor) Serial.print(" IN: ");
	if (debcnt == 20*dbfaktor) Serial.print(gyro_roll_input);
	if (debcnt == 22*dbfaktor) Serial.print(" E1d: ");
	if (debcnt == 24*dbfaktor) Serial.print(esc_4-esc_1);
	if (debcnt == 26*dbfaktor) Serial.print(" P: ");
	if (debcnt == 28*dbfaktor) Serial.print(pid_p_gain_roll * pid_error_temp);
	if (debcnt == 30*dbfaktor) Serial.print(" I: ");
	if (debcnt == 32*dbfaktor) Serial.print(pid_i_mem_roll);
	if (debcnt == 34*dbfaktor) Serial.print(" D: ");
	if (debcnt == 36*dbfaktor) Serial.print(pid_d_gain_roll * (pid_error_temp - pid_last_roll_d_error));
	if (debcnt == 38*dbfaktor) Serial.print(" ch6:");
	if (debcnt == 40*dbfaktor) Serial.print(receiver_input_channel_6);
	
	
	debcnt ++;
	if (debcnt == 100*dbfaktor) 
	{
		debcnt =0;
		Serial.println("");
	}
...
ISR(PCINT2_vect){
	...
	current_timeV2 = micros();
	if (PIND & B00000100){
		if(last_channel_6 == 0){                                   
			last_channel_6 = 1;                                    
			timer_6 = current_timeV2;                              
		}
	}
	else
	if(last_channel_6 == 1){                                       
		last_channel_6 = 0;                                        
		receiver_input[6] = current_timeV2 - timer_6;              
	}

	if(PIND & B00001000){
		if(last_channel_7 == 0){                                   
			last_channel_7 = 1;                                    
			timer_7 = current_timeV2;                              
		}
	}
	else if(last_channel_7 == 1){                                  
		last_channel_7 = 0;                                        
		receiver_input[7] = current_timeV2 - timer_7;              
	}
}

ISR(PCINT0_vect){
	current_timeV1 = micros();
	//Channel 1=========================================
	if(PINB & B00000001){                                                     //Is input 8 high?
		if(last_channel_1 == 0){                                                //Input 8 changed from 0 to 1.
			last_channel_1 = 1;                                                   //Remember current input state.
			timer_1 = current_timeV1 ;                                               //Set timer_1 to current_time.
		}
	}
	else if(last_channel_1 == 1){                                             //Input 8 is not high and changed from 1 to 0.
		last_channel_1 = 0;                                                     //Remember current input state.
		receiver_input[1] = current_timeV1  - timer_1;                             //Channel 1 is current_time - timer_1.
	}
	//Channel 2=========================================
	if(PINB & B00000010 ){                                                    //Is input 9 high?
		if(last_channel_2 == 0){                                                //Input 9 changed from 0 to 1.
			last_channel_2 = 1;                                                   //Remember current input state.
			timer_2 = current_timeV1 ;                                               //Set timer_2 to current_time.
		}
	}
	else if(last_channel_2 == 1){                                             //Input 9 is not high and changed from 1 to 0.
		last_channel_2 = 0;                                                     //Remember current input state.
		receiver_input[2] = current_timeV1  - timer_2;                             //Channel 2 is current_time - timer_2.
	}
	//Channel 3=========================================
	if(PINB & B00000100 ){                                                    //Is input 10 high?
		if(last_channel_3 == 0){                                                //Input 10 changed from 0 to 1.
			last_channel_3 = 1;                                                   //Remember current input state.
			timer_3 = current_timeV1 ;                                               //Set timer_3 to current_time.
		}
	}
	else if(last_channel_3 == 1){                                             //Input 10 is not high and changed from 1 to 0.
		last_channel_3 = 0;                                                     //Remember current input state.
		receiver_input[3] = current_timeV1  - timer_3;                             //Channel 3 is current_time - timer_3.
	...
	if(PINB & B00001000 ){                                                    //Is input 11 high?
		if(last_channel_4 == 0){                                                //Input 11 changed from 0 to 1.
			...
			timer_4 = current_timeV1 ;                                               //Set timer_4 to current_time.
		}
	}
	else if(last_channel_4 == 1){                                             //Input 11 is not high and changed from 1 to 0.
		last_channel_4 = 0;                                                     //Remember current input state.
		receiver_input[4] = current_timeV1  - timer_4;                             //Channel 4 is current_time - timer_4.
	}
	//Channel 5=========================================
	if(PINB & B00010000 ){                                                    //Is input 11 high?
		if(last_channel_5 == 0){                                                //Input 11 changed from 0 to 1.
			last_channel_5 = 1;                                                   //Remember current input state.
			timer_5 = current_timeV1 ;                                               //Set timer_4 to current_time.
		}
	}
	else if(last_channel_5 == 1){                                             //Input 11 is not high and changed from 1 to 0.
		last_channel_5 = 0;                                                     //Remember current input state.
		receiver_input[5] = current_timeV1  - timer_5;                             //Channel 4 is current_time - timer_4.
	}
